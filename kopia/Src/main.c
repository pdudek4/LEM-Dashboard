...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	HAL_UART_Receive_IT(&huart2, Uart2_buf_rx, 7);	//odbior z nextiona
  ...
  while (1)
  {
		...
		case IDLE:
		  IdleRun(&dash_state);
			break;
		case NEXTION_P:
			Nextion_PRun(&dash_state, buf_nxt_p, &nx_dowysylki_p);
			break;
		case NEXTION_R:
			Nextion_RRun(&dash_state, buf_nxt_r, &nx_dowysylki_r);
			break;
		case NEXTION_SD:
			//SDInit(&sd_card);
			Nextion_SDRun(&dash_state);
			break;
		}
